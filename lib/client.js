// dsh-theme-engine — browser half (client plugin bundle).
//
// Loaded by dsh-client-modules at /plugins/dsh-theme-engine/client.js and
...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		let _runtime_client;
		try {
			_runtime_client = require("@deepseek-ai/dsh-client-store");
		} catch (err) {
			// Only fall back on a module-table miss — the message every host
			// generation emits for an unknown seed. Any other failure (e.g. the
			// store bundle itself crashing on a future master) must stay loud: a
			// silent fallback would resurface as a misleading "runtime/client not
			// found". Avoid `instanceof Error` — the factory may run in a different
			// VM realm than the throwing loader.
			const msg = err && err.message;
			if (typeof msg !== "string" || !msg.includes("missed the module table")) throw err;
			_runtime_client = require("@deepseek-ai/dsh-client-runtime/client");
		}

		//#region dsh-theme-engine: constants & presets
		/** The settings row's locale namespace. */
		const SETTINGS_NS = "settings.dreamSkin";
		/** localStorage key holding the selected skin id. */
		const STORAGE_KEY = "dsh-theme-engine:skin";
		/** localStorage key holding the wallpaper image (data URL). */
		const WALLPAPER_KEY = "dsh-theme-engine:wallpaper";
		/** localStorage key holding the wallpaper wash opacity (0..1). */
		const WALLPAPER_OPACITY_KEY = "dsh-theme-engine:wallpaper-opacity";
		/** localStorage key holding the wallpaper blur radius (px). */
		const WALLPAPER_BLUR_KEY = "dsh-theme-engine:wallpaper-blur";
		/** localStorage key holding recent wallpaper history (JSON array of {kind,value}). */
		const WALLPAPER_HISTORY_KEY = "dsh-theme-engine:wallpaper-history";
		...
		const SIDEBAR_OPACITY_KEY = "dsh-theme-engine:sidebar-opacity";
		...
		const SIDEBAR_LINK_KEY = "dsh-theme-engine:sidebar-link";
		...
		const MODAL_OPACITY_KEY = "dsh-theme-engine:modal-opacity";
		...
		const MODAL_FILL_VAR = "--dsh-theme-engine-modal-fill";
		/**
		 ...
		 */
		const BUILTIN_LAST_KEY = "dsh-theme-engine:builtin-last";
		...
		const SKINS = [
			{
				...
				tokens: {
					...
					"--dsw-specific-input-major": "rgba(255, 255, 255, 1.00)",
		...
		//#region dsh-theme-engine: persistence (host-backed, origin-independent)
		/**
		...
		//#region dsh-theme-engine: wallpaper layer + token shading
		 ...
		 */
		const COMBINED_OVERRIDE_SOURCE = "dsh-theme-engine:appearance";
		...
		//#region dsh-theme-engine: liquid-glass material CSS
		/**
		 ...
		 */
		const MATERIAL_CSS_SOURCE = "dsh-theme-engine:material:liquid-glass";
		...
		//#region dsh-theme-engine: image compression
		/**
		...
		//#region dsh-theme-engine: settings row stores
		/**
		...
		//#region dsh-theme-engine: settings rows
		...
		//#region dsh-theme-engine: P0 shared utilities (packs, accent, persistence, random)
		/**
		...
		const PACK_FORMAT = "dsh-theme-engine/pack";
		...
		const PACKS_KEY = "dsh-theme-engine:packs"; // JSON array of remote/manual pack manifests
		const ACCENT_KEY = "dsh-theme-engine:accent"; // hex accent (#rrggbb) or "system"
		const FAVORITES_KEY = "dsh-theme-engine:favorites"; // JSON array of theme/ pack ids
		const WALLPAPER_URL_KEY = "dsh-theme-engine:wallpaper-url";
		const WALLPAPER_KIND_KEY = "dsh-theme-engine:wallpaper-kind"; // 'image'|'url'|'gradient'
		const WALLPAPER_GRADIENT_KEY = "dsh-theme-engine:wallpaper-gradient";
		const WALLPAPER_AUTODIM_KEY = "dsh-theme-engine:wallpaper-autodim"; // '1'|'0'
		...
		const WALLPAPER_FOLLOWS_SKIN_KEY = "dsh-theme-engine:wallpaper-follows-skin";
		...
		function exportPackAsFile(ctx, id) {
			...
			a.href = url;
			a.download = (source.manifest.name || id).toLowerCase().replace(/\s+/g, "-") + ".dsh-theme-engine.json";
		...
		//#region dsh-theme-engine: P0 favorites + surprise-me
		...
		//#region dsh-theme-engine: P0 stores + module hooks
		...
		//#region dsh-theme-engine: P0 accent override
		...
		//#region dsh-theme-engine: P0 wallpaper 2.0 (url / gradient / auto-dim)
		...
		//#region dsh-theme-engine: P0 share-url import
		...
		//#region dsh-theme-engine: P0 UI rows (accent + packs)
					...
					(0, react_jsx_runtime.jsxs)("div", {
						...
						children: [
							...
							(0, react_jsx_runtime.jsx)("input", {
								ref: fileInput,
								type: "file",
								accept: ".json,.dsh-theme-engine.json,.dsh-theme-engine,application/json",
								style: { display: "none" },
								onChange: onFile
							}),
							(0, react_jsx_runtime.jsx)("button", {
		...
		//#region dsh-theme-engine: Appearance settings section
		/**
		...
		//#endregion
		//#region dsh-theme-engine: client plugin body
		/**
		...
		function apply(ctx) {
			...
			}, "dsh-theme-engine: theme registration");
			...
			}, "dsh-theme-engine: sticky skin + built-in restore");
			...
			ctx.effect(() => () => {
				...
				combinedOverrideDispose?.();
				combinedOverrideDispose = null;
			}, "dsh-theme-engine: wallpaper + material + packs cleanup");
			...
			ctx.effect(() => () => {
				if (wallpaperReshadeTimer !== null) clearTimeout(wallpaperReshadeTimer);
			}, "dsh-theme-engine: deferred wallpaper re-shade");
			...
			ctx.effect(() => ctx.locale.register(SETTINGS_NS, {
				...
				de,
				ru
			}), "dsh-theme-engine: settings row dictionaries");
...
;(function () {
  var MARKER = "data-dsh-theme-engine-nav"
  ...
  style.id = "dsh-theme-engine-nav-icon"
